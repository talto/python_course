...
def _get_metadata(package_name):
    try:
        return json.loads(
            pkg_resources.get_distribution(package_name).get_metadata("pbr.json")
        )
...
def main():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=str(pbr.version.VersionInfo("pbr"))
    )
    ...
    cmd_info.add_argument(
        "-s", "--short", action="store_true", help="only display package version"
    )
