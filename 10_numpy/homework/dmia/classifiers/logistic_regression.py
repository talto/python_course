...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_proba = np.vstack([prob_neg, prob_pos]).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        data_loss = -np.mean(
            y_batch * np.log(probs + 1e-12) +
            (1 - y_batch) * np.log(1 - probs + 1e-12)
        )
        w_no_bias = self.w.copy()
        w_no_bias[-1] = 0.0
        reg_loss = 0.5 * reg * np.sum(w_no_bias ** 2)
        loss = data_loss + reg_loss
        dscores = (probs - y_batch) / N
        dw = X_batch.T.dot(dscores) + reg * w_no_bias
        return loss, dw
